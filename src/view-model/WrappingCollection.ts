...
import { getDefaultBindingEngine, getDefaultObserverLocator } from '../core';
...
export class WrappingCollection extends Array {
    ...
    private _bindingEngine: BindingEngine = getDefaultBindingEngine();
    ...
    constructor ( factoryMethod?: (item: IModel<any>) => ObjectViewModel<IModel<any>>
      ...
      this._bindingEngine = getDefaultBindingEngine();
    ...
    private onSubscribe: (changes: any) => void = (changes: any) => {
        ...
        if ((<Array<any>>changes).length === 0) {
                ...
                if (innerChanges.addedCount === 1) {
                    ...
                    this.addCore(this._source[innerChanges.index],
                                WrappingCollection.createWrapper(
                                  this._source[innerChanges.index],
                                  this.factoryMethod), innerChanges.index);
                } else if (innerChanges.addedCount > 1) {
                    for (let i = 0; i < innerChanges.addedCount; i++) {
                        this.addCore(this._source[innerChanges.index + i],
                                    WrappingCollection.createWrapper(this._source[innerChanges.index + i],
                                      this.factoryMethod), innerChanges.index + i);
    ...
    private clbk(changes: any) {
        console.log('CHANGES 2:  ' + typeof changes);
    }

    private static createWrapper(item: any, factoryMethod: any): ObjectViewModel<IModel<any>> | any {
